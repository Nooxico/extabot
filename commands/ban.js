...
module.exports.run = async (bot, message, args) => {
    ...
    if (member.roles.cache.some(role => role.name === '*'))
        return message.channel.send("Vous ne pouvez pas bannir ce joueur.");
    ...
    let transcriptembed = new Discord.MessageEmbed()
        ...
        .setAuthor('Membre banni', (message.author.displayAvatarURL({ dynamic: true, size: 512 })))
        .addFields(
            { name: ':bust_in_silhouette: Membre :', value: `${member}`, inline: true },
            { name: ':hash: Banni par:', value: `${message.author.username}`, inline: true },
        )
        .addField('Raison', `${reason}`)
        .setColor('E74C3C')
        .setFooter(`ID: ${member.id} ● Copyright 2021-2022 © Nooxico#0001`)
